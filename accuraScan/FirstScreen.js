	...
	getAccuraSetup = () => {
		...
		AccurascanKyc.getMetaData((error, response) => {
			if (error != null) {
				...
				setUpCustomMessages()
				console.log(
					'response from AccuraScan getMetaData JS method -',
					response,
				)
				...
				;(objSDKRes = res), (ocrContries = newContries)
		...
		AccurascanKyc.setupAccuraConfig(
			...
			(error, response) => {
				if (error != null) {
					...
					console.log(
						'response from AccuraScan JS setupAccuraConfig method - ',
						response,
					)
				}
			},
				...
				<>
					{
						<SafeAreaView>
							<View style={styles.container}>
								...
								<TouchableOpacity
									onPress={() =>
										navigation.navigate('Facematch Screen')
									}
								>
									<Image
